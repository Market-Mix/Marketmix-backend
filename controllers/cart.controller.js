...
const addToCart = async (req, res) => {
  try {
    ...
    const specsText = specs ? JSON.stringify(specs) : null;

    console.log('🛒 Add to cart request:', { product_id, quantity, color, size, selected_specifications: specs, specsText, user_id });
    ...
    const existingCartItem = await db.query(
      `SELECT id, quantity, color, size, selected_specifications FROM cart_items 
       WHERE cart_id = $1 AND product_id = $2
         ...
         AND COALESCE(selected_specifications::text, '__null__') = COALESCE($5, '__null__')
       LIMIT 1`,
      [cartId, product_id, color, size, specsText]
    );
...
const normalizedItems = normalizeCartItems(items);
    ...
    for (const item of normalizedItems) {
      ...
      const itemSpecsText = item.selected_specifications ? JSON.stringify(item.selected_specifications) : null;
      const existingCartItem = await client.query(
        `SELECT id, quantity, color, size, selected_specifications FROM cart_items
         WHERE cart_id = $1 AND product_id = $2
           ...
           AND COALESCE(selected_specifications::text, '__null__') = COALESCE($5, '__null__')
         FOR UPDATE`,
        [cartId, item.product_id, item.color, item.size, itemSpecsText]
      );
